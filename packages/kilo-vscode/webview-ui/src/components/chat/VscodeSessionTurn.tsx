...
export const VscodeSessionTurn: Component<VscodeSessionTurnProps> = (props) => {
  ...
  createEffect(() => {
    if (!speechSettings().enabled || !speechSettings().autoSpeakAssistant) return
    const text = assistantPlainText()
    const signature = assistantSpeechSignature()
    if (!text || session.status() !== "idle" || !signature || signature === lastAutoSpokenSignature) return
    lastAutoSpokenSignature = signature
    void speechController.speak(text, speechSettings())
  })
