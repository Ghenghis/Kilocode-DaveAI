...
export class KiloProvider implements vscode.WebviewViewProvider, TelemetryPropertiesProvider {
    ...
    this.webviewMessageDisposable = webview.onDidReceiveMessage(async (message) => {
      ...
      switch (message.type) {
        ...
        case "requestSpeechSettings":
          await this.sendSpeechSettings()
          break
        case "saveSpeechSecret":
          await this.saveSpeechSecret(message.secretKey, message.value)
          break
        case "clearSpeechSecret":
          await this.clearSpeechSecret(message.secretKey)
          break
        case "synthesizeSpeech":
          await this.handleSynthesizeSpeech(message.requestId, message.provider, message.text)
          break
  ...
  private get speechSecrets() {
    return {
      azureApiKey: "kilo-code.new.speech.azureApiKey",
      openaiCompatibleApiKey: "kilo-code.new.speech.openaiCompatibleApiKey",
    } as const
  }

  private async saveSpeechSecret(secretKey: "azureApiKey" | "openaiCompatibleApiKey", value: string): Promise<void> {
    await this.extensionContext?.secrets.store(this.speechSecrets[secretKey], value)
    this.sendSpeechSettings()
  }

  private async clearSpeechSecret(secretKey: "azureApiKey" | "openaiCompatibleApiKey"): Promise<void> {
    await this.extensionContext?.secrets.delete(this.speechSecrets[secretKey])
    this.sendSpeechSettings()
  }

  private async sendSpeechSettings(): Promise<void> {
    const speech = vscode.workspace.getConfiguration("kilo-code.new.speech")
    const [azureApiKey, openaiApiKey] = await Promise.all([
      this.extensionContext?.secrets.get(this.speechSecrets.azureApiKey),
      this.extensionContext?.secrets.get(this.speechSecrets.openaiCompatibleApiKey),
    ])
    this.postMessage({
      type: "speechSettingsLoaded",
      settings: {
        enabled: speech.get<boolean>("enabled", true),
        autoSpeakAssistant: speech.get<boolean>("autoSpeakAssistant", false),
        location: speech.get<"local" | "cloud">("location", "local"),
        provider: speech.get<"browser" | "azure" | "openaiCompatible">("provider", "browser"),
        hasAzureKey: Boolean(azureApiKey),
        hasOpenAICompatibleKey: Boolean(openaiApiKey),
        browserVoice: speech.get<string>("browserVoice", ""),
        browserLang: speech.get<string>("browserLang", "en-GB"),
        browserRate: speech.get<number>("browserRate", 1),
        browserPitch: speech.get<number>("browserPitch", 1),
        azureRegion: speech.get<string>("azureRegion", "westus"),
        azureVoice: speech.get<string>("azureVoice", "en-GB-MaisieNeural"),
        azureFormat: speech.get<string>("azureFormat", "audio-24khz-48kbitrate-mono-mp3"),
        openaiCompatibleBaseUrl: speech.get<string>("openaiCompatibleBaseUrl", "http://127.0.0.1:8000/v1"),
        openaiCompatibleModel: speech.get<string>("openaiCompatibleModel", "tts-1"),
        openaiCompatibleVoice: speech.get<string>("openaiCompatibleVoice", "alloy"),
        openaiCompatibleResponseFormat: speech.get<string>("openaiCompatibleResponseFormat", "mp3"),
      },
    })
  }

  private async handleSynthesizeSpeech(
    requestId: string,
    provider: "azure" | "openaiCompatible",
    text: string,
  ): Promise<void> {
    try {
      const speech = vscode.workspace.getConfiguration("kilo-code.new.speech")
      if (provider === "azure") {
        const apiKey = await this.extensionContext?.secrets.get(this.speechSecrets.azureApiKey)
        if (!apiKey) throw new Error("Azure Speech API key is not configured")
        const region = speech.get<string>("azureRegion", "westus")
        const voice = speech.get<string>("azureVoice", "en-GB-MaisieNeural")
        const format = speech.get<string>("azureFormat", "audio-24khz-48kbitrate-mono-mp3")
        const body = `<speak version='1.0' xml:lang='en-GB'><voice name='${voice}'>${this.escapeXml(text)}</voice></speak>`
        ...
        this.postMessage({ type: "speechAudioChunk", requestId, mimeType: "audio/mpeg", data })
        return
      }

      const apiKey = await this.extensionContext?.secrets.get(this.speechSecrets.openaiCompatibleApiKey)
      const baseUrl = speech.get<string>("openaiCompatibleBaseUrl", "http://127.0.0.1:8000/v1").replace(/\/$/, "")
      const model = speech.get<string>("openaiCompatibleModel", "tts-1")
      const voice = speech.get<string>("openaiCompatibleVoice", "alloy")
      const responseFormat = speech.get<string>("openaiCompatibleResponseFormat", "mp3")
